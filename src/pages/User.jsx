...
const User = () => {
  ...
  if (!user) return <div>Không thể tải thông tin người dùng.</div>;
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="flex flex-col items-center justify-center md:w-1/3 border-r border-gray-100 pr-6">
            ...
            <h4 className="text-lg font-bold text-gray-900">{user.fullName || "Chưa cập nhật"}</h4>
            <p className="text-xs text-gray-400 mt-1">Tham gia: {user.createdAt ? new Date(user.createdAt).toLocaleDateString() : "N/A"}</p>
          </div>
          ...
          <div className="md:w-2/3 space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium text-gray-700 mt-1">{user.dob ? new Date(user.dob).toLocaleDateString() : "Chưa cập nhật"}</p>
              </div>
              <div>
                <label className="text-xs text-gray-400 uppercase font-bold tracking-wider">Giới tính</label>
                <p className="font-medium text-gray-700 mt-1">{user.gender || "Chưa cập nhật"}</p>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex justify-between items-center bg-gray-50 p-3 rounded-lg mt-1 border border-gray-100">
                <span className="truncate text-sm font-medium text-gray-700">{user.phone || "Chưa cập nhật"}</span>
              </div>
            </div>
          </div>
        </div>

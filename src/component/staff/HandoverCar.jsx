...
const HandoverCar = () => {
  ...
  const bookingInfo = {
    id: "1", // Khớp với id trong danh sách Booking
    customerName: "Nguyễn Văn A",
    phone: "0912345678",
    carName: "Toyota Camry",
    licensePlate: "ABC-123",
    rentalPeriod: "01/02/2024 - 05/02/2024",
  };
        ...
        <div className="md:col-span-1 space-y-4">
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
            <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
              <FileText size={18} className="text-blue-600"/> Đơn #{bookingInfo.id}
            </h3>
            <div className="space-y-3 text-sm text-gray-600">
              <p className="flex items-center gap-2"><User size={16}/> <strong>Khách:</strong> {bookingInfo.customerName}</p>
              <p className="flex items-center gap-2"><Car size={16}/> <strong>Xe:</strong> {bookingInfo.carName}</p>
              <p className="flex items-center gap-2 text-blue-600 font-semibold border border-blue-100 bg-blue-50 w-fit px-2 py-1 rounded">
                Biển số: {bookingInfo.licensePlate}
              </p>
              <p className="flex items-center gap-2"><Calendar size={16}/> <strong>Thời gian:</strong> <br/>{bookingInfo.rentalPeriod}</p>
            </div>
          </div>
        </div>
